import httpx
from typing import Any, Dict, Optional, Union, Literal, List, overload, Tuple
...
class AsyncOMDClient:
    ...
    def __init__(
        ...
        *,
        ...
        if base_client:
            ...
            transport = transport or httpx.AsyncHTTPTransport(retries=self.config.retries)
    ...
    @overload
    async def get(self, endpoint: str, *, params: Optional[Dict[str, Any]] = ..., nullable: Literal[True]) -> Optional[Dict[str, Any]]: ...
    @overload
    async def get(self, endpoint: str, *, params: Optional[Dict[str, Any]] = ..., nullable: Literal[False] = ...) -> Dict[str, Any]: ...
    async def get(
        self,
        endpoint: str,
        *,
        ...
        try:
            ...
            self.logger.exception("Unexpected error on GET %s", url)
            raise
    
    async def get_table_by_name(self, fqn: str, fields: str = "*", nullable: bool = False) -> Optional[Dict[str, Any]]:
        params = {"feilds": fields}
        return await self.get(endpoint=f"/tables/name/{fqn}", params=params, nullable=nullable)

    async def get_table_by_id(self, id: str, fields: str = "*", nullable: bool = False) -> Optional[Dict[str, Any]]:
        params = {"feilds": fields}
        return await self.get(endpoint=f"/tables/{id}", params=params, nullable=nullable)
    ...
    async def add_sample_data(self, id: str, body: SampleData) -> Dict[str, Any]:
        return await self.put(endpoint=f"/tables/{id}/sampleData", json=body.model_dump(mode="json"))
    ...
    async def patch(
        ...
        *,
        ...
        url = self._build_url(endpoint)
        try:
            response = await self._client.patch(url, json=json, data=data, params=params)
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def delete(
        ...
        *,
        ...
        url = self._build_url(endpoint)
        try:
            response: httpx.Response = await self._client.delete(url, json=json, params=params)
